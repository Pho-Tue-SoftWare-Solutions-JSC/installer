...
function log() {
    ...
    message="[HiTechCloud ${timestamp} install Log]: $1 "
    case "$1" in
        *"$TXT_RUN_AS_ROOT"*)
...
function Prepare_System(){
    if which hitechcloud >/dev/null 2>&1; then
...
function Set_Dir(){
    ...
    if [[ -f "$PANEL_BASE_DIR/hitechcloud/db/core.db" ]]; then
        USE_EXISTING=true
    fi
}
...
BACKUP_FILE="/etc/docker/daemon.json.hitechcloud_bak"
...
function Generate_API_Key(){
    ...
    HITECHCLOUD_CONF_DIR="/etc/hitechcloud"

    mkdir -p "$HITECHCLOUD_CONF_DIR"
    echo "$API_KEY" > "$HITECHCLOUD_CONF_DIR/api.key"
    chmod 600 "$HITECHCLOUD_CONF_DIR/api.key"

    log ""
    log "================================================================"
    log "HiTechCloud API Key: $API_KEY"
    log "Stored at: $HITECHCLOUD_CONF_DIR/api.key"
    log "================================================================"
    log ""
}
...
init_configure() {
    cp ./hitechcloud-core /usr/local/bin && chmod +x /usr/local/bin/hitechcloud-core
    if [[ -e /usr/bin/hitechcloud ]]; then
        rm -f /usr/bin/hitechcloud
    fi
    ln -s /usr/local/bin/hitechcloud-core /usr/bin/hitechcloud >/dev/null 2>&1
    if [[ ! -f /usr/bin/hitechcloud-core ]]; then
        ln -s /usr/local/bin/hitechcloud-core /usr/bin/hitechcloud-core >/dev/null 2>&1
    fi

    cp ./hitechcloud-agent /usr/local/bin && chmod +x /usr/local/bin/hitechcloud-agent
    if [[ ! -f /usr/bin/hitechcloud-agent ]]; then
        ln -s /usr/local/bin/hitechcloud-agent /usr/bin/hitechcloud-agent >/dev/null 2>&1
    fi
...
install_and_configure() {
    if command -v systemctl &>/dev/null; then
        init_configure
        cp ./initscript/hitechcloud-core.service /etc/systemd/system
        cp ./initscript/hitechcloud-agent.service /etc/systemd/system
        systemctl enable hitechcloud-agent.service; systemctl enable hitechcloud-core.service; systemctl daemon-reload 2>&1 | tee -a ${LOG_FILE}
        log "$TXT_START_PANEL_SERVICE"
        systemctl start hitechcloud-core | tee -a ${LOG_FILE}
        systemctl start hitechcloud-agent | tee -a ${LOG_FILE}
    else
     	mkdir -p /usr/local/bin
	    init_configure
        if [ -f /etc/rc.common ]; then
            cp ./initscript/hitechcloud-core.procd /etc/init.d/hitechcloud-core
            cp ./initscript/hitechcloud-agent.procd /etc/init.d/hitechcloud-agent
            chmod +x /etc/init.d/hitechcloud-core
            chmod +x /etc/init.d/hitechcloud-agent
            /etc/init.d/hitechcloud-core enable | tee -a ${LOG_FILE}
            /etc/init.d/hitechcloud-agent enable | tee -a ${LOG_FILE}
        elif [ -f /sbin/openrc-run ]; then
            cp ./initscript/hitechcloud-core.openrc /etc/init.d/hitechcloud-core
            cp ./initscript/hitechcloud-agent.openrc /etc/init.d/hitechcloud-agent
            chmod +x /etc/init.d/hitechcloud-core
            chmod +x /etc/init.d/hitechcloud-agent
            rc-update add hitechcloud-core default 2>&1 | tee -a ${LOG_FILE}
            rc-update add hitechcloud-agent default 2>&1 | tee -a ${LOG_FILE}
        else
            cp ./initscript/hitechcloud-core.init /etc/init.d/hitechcloud-core
            cp ./initscript/hitechcloud-agent.init /etc/init.d/hitechcloud-agent
            chmod +x /etc/init.d/hitechcloud-core
            chmod +x /etc/init.d/hitechcloud-agent
            /etc/init.d/hitechcloud-core enable | tee -a ${LOG_FILE}
            /etc/init.d/hitechcloud-agent enable | tee -a ${LOG_FILE}
        fi
        /etc/init.d/hitechcloud-core start | tee -a ${LOG_FILE}
        /etc/init.d/hitechcloud-agent start | tee -a ${LOG_FILE}
    fi
}
...
function Init_Panel(){
    ...
    MAX_ATTEMPTS=5
    RUN_BASE_DIR=$PANEL_BASE_DIR/hitechcloud
    ...
    for attempt in $(seq 1 $MAX_ATTEMPTS); do
        if command -v systemctl >/dev/null 2>&1; then
            core_status=$(systemctl status hitechcloud-core 2>&1 | grep Active)
            agent_status=$(systemctl status hitechcloud-agent 2>&1 | grep Active)
            if [[ "$core_status" == *running* && "$agent_status" == *running* ]]; then
                log "$TXT_PANEL_SERVICE_START_SUCCESS"
                break
            fi
        elif command -v opkg >/dev/null 2>&1; then
            core_status=$(/etc/init.d/hitechcloud-core status 2>&1)
            agent_status=$(/etc/init.d/hitechcloud-agent status 2>&1)
            if [[ "$core_status" == *running* && "$agent_status" == *running* ]]; then
                log "$TXT_PANEL_SERVICE_START_SUCCESS"
                break
            fi
        else
            core_status=$(service hitechcloud-core status >/dev/null 2>&1 && echo active || echo inactive)
            agent_status=$(service hitechcloud-agent status >/dev/null 2>&1 && echo active || echo inactive)
...
function Check_Ready() {
    ...
    if [ ! -e /etc/hitechcloud/agent.sock ]; then
