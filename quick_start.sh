...
GITHUB_ORG="Pho-Tue-SoftWare-Solutions-JSC"
GITHUB_REPO="hitechcloud-agent"
...
PACKAGE_FILE_NAME="hitechcloud-${VERSION}-linux-${architecture}.tar.gz"
...
if [[ -f ${PACKAGE_FILE_NAME} ]]; then
    ...
    if [[ "$EXPECTED_HASH" == "$actual_hash" ]]; then
        ...
        rm -rf hitechcloud-${VERSION}-linux-${architecture}
        tar zxf ${PACKAGE_FILE_NAME}
        cd hitechcloud-${VERSION}-linux-${architecture}
...
echo "Preparing to download HitechCloud ${VERSION} (${architecture}, mode: ${INSTALL_MODE})."
